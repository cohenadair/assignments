...
void CubeFace::rotateSidesHelper(int p[][3][2], int angle, int orientationAngles[]) {
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < kGridSize; j++)
            sideFaces[i]->getStickers()[p[i][j][0]][p[i][j][1]]->rotate(orientationAngles[i]);
    }
...
// draws the black square that appears behind the grid
void CubeFace::drawBackdrop(double xRot, double yRot, double zRot) {
    glLoadIdentity();
    ...
    glPushMatrix();
        faceFront();
        glColor3f(0, 0, 0);
        glTranslatef(0, 0, kCubeRadius);
        glBegin(GL_QUADS);
            glVertex3f(kCubeRadius, kCubeRadius, 0);
            glVertex3f(-kCubeRadius, kCubeRadius, 0);
            glVertex3f(-kCubeRadius, -kCubeRadius, 0);
            glVertex3f(kCubeRadius, -kCubeRadius, 0);
        glEnd();
...
void CubeFace::drawStickers(int faceIndex) {
    faceFront();
    for (int r = 0; r < kGridSize; r++)
        for (int c = 0; c < kGridSize; c++)
            stickers[r][c]->draw(r, c);
}
