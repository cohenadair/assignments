...
bool UnixinitializeNetwork(void)
{
	// if the signal is already initialized, don't do it again
	if (!socketFactory.initialized) {
...
nmu_socket_t UnixAllocSocket(void)
{
	...
	pSocket->acceptAndForkService = UnixAcceptAndForkService;
	return pSocket;
}
...
void UnixAcceptAndForkService(nmu_socket_t pServerSocket, pFunVS protocol) {
	...
	serviceSocket = pServerSocket->accept(pServerSocket);
	childPid = fork();
	...
	if(childPid){ // The parent doesn't need the service socket.
		...
		pServerSocket->close(pServerSocket);
